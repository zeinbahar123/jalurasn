...
export default async function HalamanHasil({
  ...
  if (!session?.user?.id) redirect(`/masuk?lanjut=/hasil/${id}`);

  const attempt = await prisma.attempt.findUnique({
    where: { id },
    include: {
      answers: {
        select: { soalId: true, jawaban: true, detikDipakai: true, raguRagu: true, benar: true },
      },
    },
  });
  ...
  const terlemah = [...analisis].sort((a, b) => a.persen - b.persen).slice(0, 3);
  const terkuat = [...analisis].sort((a, b) => b.persen - a.persen).slice(0, 3);

  const dikosongkan = soal.filter((s) => !petaJawaban[s.id]).length;
  const raguCount = attempt.answers.filter((a) => a.raguRagu).length;

  // Posisi relatif terhadap peserta lain di paket yang sama
  const [totalPeserta, dibawahAnda, rerata] = await Promise.all([
    prisma.attempt.count({ where: { paketId: paket.id, status: "SELESAI" } }),
    prisma.attempt.count({
      where: { paketId: paket.id, status: "SELESAI", skorTotal: { lt: attempt.skorTotal } },
    }),
  ...
  const persentil =
    totalPeserta > 1 ? Math.round((dibawahAnda / totalPeserta) * 100) : null;
  ...
  const gagalDi = URUTAN_KATEGORI.filter((k) => !lulusKategori[k]);
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-10 sm:px-6">
      ...
      <Card className="mt-5 overflow-hidden">
        ...
        >
          ...
          <p className="mt-1 text-sm font-medium text-white/70">
            dari {SKOR_MAKS_TOTAL} · ambang batas kumulatif {AMBANG_TOTAL}
          </p>

          <div className="mt-6 inline-flex items-center gap-2 rounded-full bg-white/15 px-4 py-2 text-sm font-bold text-white">
            {attempt.lulusSemua ? (
              <>
                <IkonCentang /> Lolos ambang batas semua komponen
              </>
            ) : (
              <>
                <IkonSilang /> Belum lolos — {gagalDi.join(" & ")} di bawah ambang
                batas
              </>
            )}
          </div>
          ...
          <p className="mt-5 text-sm text-white/70">
            Waktu pengerjaan {formatDurasiPanjang(attempt.durasiDetik)}
            {persentil !== null
        ...
        <div className="grid gap-px bg-slate-200 sm:grid-cols-3 dark:bg-slate-800">
          {URUTAN_KATEGORI.map((kat) => {
            ...
            return (
              <div key={kat} className="bg-white p-5 dark:bg-slate-900">
                ...
                <p className="mt-2 text-xs text-slate-500 dark:text-slate-400">
                  ...
                  {kat !== "TKP"
                    ? ` · benar ${kat === "TWK" ? attempt.benarTWK : attempt.benarTIU}/${a.jumlahSoal}`
                    : ` · rata-rata ${(skor / a.jumlahSoal).toFixed(2)} poin/soal`}
                </p>
              </div>
            );
          })}
      ...
      <div className="mt-5 flex flex-wrap gap-3">
        <ButtonLink href={`/pembahasan/${attempt.id}`} ukuran="lg">
          Buka Pembahasan Lengkap
        </ButtonLink>
        <ButtonLink href={`/tryout/${paket.slug}`} varian="sekunder" ukuran="lg">
          Kerjakan Ulang
        </ButtonLink>
        <ButtonLink href="/peringkat" varian="sekunder" ukuran="lg">
            ...
            {terlemah.map((r) => (
              <li key={`${r.kategori}-${r.sub}`}>
                ...
                <div className="mt-1.5">
                  <Meter nilai={r.poin} maks={r.poinMaks} warna="rose" />
                </div>
          ...
          <ul className="mt-4 space-y-4">
            {terkuat.map((r) => (
              <li key={`${r.kategori}-${r.sub}`}>
                ...
                <div className="mt-1.5">
                  <Meter nilai={r.poin} maks={r.poinMaks} warna="emerald" />
                </div>
                <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
                  {r.benar} dari {r.jumlah} soal terjawab baik
                </p>
      ...
      <Card className="mt-5 overflow-hidden">
        <div className="border-b border-slate-200 px-5 py-4 dark:border-slate-800">
          ...
          <p className="mt-0.5 text-sm text-slate-500 dark:text-slate-400">
            Pada TKP, &quot;terjawab baik&quot; berarti Anda memilih opsi bernilai
            4 atau 5.
          </p>
        </div>
          ...
          <table className="w-full text-sm">
            <thead className="bg-slate-50 text-left text-xs tracking-wider text-slate-500 uppercase dark:bg-slate-800/50">
              <tr>
                ...
                <th className="px-5 py-3 font-semibold">Sub-materi</th>
                <th className="px-5 py-3 text-center font-semibold">Soal</th>
                <th className="px-5 py-3 text-center font-semibold">Baik</th>
                <th className="px-5 py-3 text-center font-semibold">Poin</th>
                <th className="px-5 py-3 font-semibold">Penguasaan</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
              {analisis.map((r) => (
                <tr key={`${r.kategori}-${r.sub}`}>
                  ...
                  <td className="px-5 py-3 text-center text-slate-500">{r.jumlah}</td>
                  <td className="px-5 py-3 text-center text-slate-500">{r.benar}</td>
        ...
        <Card className="p-5">
          ...
          <p className="mt-1.5 text-xs leading-5 text-slate-500 dark:text-slate-400">
            {dikosongkan === 0
              ...
              : `Potensi hilang sampai ${dikosongkan * 5} poin. Di SKD tidak ada nilai minus — selalu tebak.`}
          </p>
        </Card>
        <Card className="p-5">
          <p className="text-xs font-semibold tracking-wider text-slate-500 uppercase">
          ...
          <p className="mt-1.5 text-xs leading-5 text-slate-500 dark:text-slate-400">
            Cek soal-soal ini di pembahasan; keraguan biasanya menandakan konsep
            yang belum utuh, bukan sekadar kurang teliti.
          </p>
        </Card>
        <Card className="p-5">
          <p className="text-xs font-semibold tracking-wider text-slate-500 uppercase">
          ...
          <p className="mt-1 text-3xl font-extrabold text-slate-900 dark:text-white">
            {rerata._avg.skorTotal ? Math.round(rerata._avg.skorTotal) : "—"}
          </p>
          <p className="mt-1.5 text-xs leading-5 text-slate-500 dark:text-slate-400">
            {totalPeserta.toLocaleString("id-ID")} peserta pernah mengerjakan
            paket ini.
          </p>
        </Card>
      </div>
      ...
      <div className="mt-6">
        {attempt.lulusSemua ? (
          <Callout nada="sukses" judul="Langkah berikutnya">
            Skor Anda sudah melewati ambang batas, tapi ingat: kelulusan CPNS
            ditentukan <strong>peringkat</strong>, bukan sekadar lolos ambang
            batas. Buka <Link href="/peringkat" className="font-semibold underline">Peta Persaingan</Link>{" "}
            untuk melihat posisi Anda di antara pejuang lain yang mengincar
            formasi serupa.
          </Callout>
        ) : (
          <Callout nada="peringatan" judul="Belum lolos — dan itu wajar">
            {gagalDi.length === 1
              ...
              : `Ada ${gagalDi.length} komponen di bawah ambang batas (${gagalDi.join(", ")}). Kerjakan satu per satu, mulai dari yang selisihnya paling kecil ke ambang batas.`}{" "}
            Buka pembahasan dan diskusikan soal yang membingungkan bersama
            peserta lain.
          </Callout>
        )}
      </div>
