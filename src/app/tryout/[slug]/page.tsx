...
export async function generateMetadata({
  ...
  const premium = paket.nomor > 1;

  const judul = premium
    ? `${paket.nama} — Try Out SKD CPNS 2026 Premium`
    : `${paket.nama} — Try Out SKD CPNS 2026 Gratis`;

  const ringkas = premium
    ? `${paket.tema}. 110 soal (TWK 30, TIU 35, TKP 45), 100 menit, lengkap dengan pembahasan tiap soal. Termasuk akses Premium TACTIX.`
    : `${paket.tema}. 110 soal (TWK 30, TIU 35, TKP 45), 100 menit, lengkap dengan pembahasan tiap soal. Gratis.`;
  ...
  return {
    ...
    keywords: [
      ...
      "latihan soal CPNS",
      premium ? "TACTIX Premium" : "try out CPNS gratis",
    ],
    alternates: {
      canonical: `/tryout/${paket.slug}`,
    },
    openGraph: {
      type: "article",
...
export default async function DetailPaket({
   ...
   * Paket 2–10    = PREMIUM
   *
   * Satu pembelian Premium membuka semua paket premium.
   */
  ...
  /*
   * Riwayat hanya diambil untuk user yang login.
   */
  const riwayat =
    userId && bolehMulai
  ...
  const sesiBerjalan = riwayat.find(
    (r) => r.status === "BERLANGSUNG",
  );

  const selesai = riwayat.filter(
    (r) => r.status !== "BERLANGSUNG",
  );

  /*
   * Statistik peserta tetap boleh ditampilkan.
   */
  ...
  // Breadcrumb untuk mesin pencari.
  const remah = {
      ...
      <div className="mt-6 grid gap-8 lg:grid-cols-[1.6fr_1fr]">
        <div>
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Badge className={WARNA_TINGKAT[paket.tingkat]}>
              {paket.tingkat}
            </Badge>

            {paketPremium ? (
              <Badge className="bg-violet-500/10 text-violet-700 ring-violet-600/20 dark:text-violet-300 dark:ring-violet-400/30">
                🔒 Premium
              </Badge>
            ) : (
              <Badge className="bg-emerald-500/10 text-emerald-700 ring-emerald-600/20 dark:text-emerald-300 dark:ring-emerald-400/30">
                🔓 Gratis
              </Badge>
            )}
          </div>
          ...
          {paketPremium && !isPremium ? (
            <Callout nada="peringatan" judul="Paket Premium TACTIX">
              Paket ini termasuk dalam akses Premium TACTIX. Dengan satu
              pembelian Premium, Anda dapat membuka seluruh Try Out 2–10.
            </Callout>
          ) : null}
                  ...
                  <div key={kat} className="px-5 py-4">
                    <div className="flex flex-wrap items-baseline justify-between gap-2">
                      ...
                      <p className="text-sm font-semibold text-slate-500">
                        {stat.perKategori[kat]} soal · maks {a.skorMaks} ·
          ...
          <Card className="mt-6 p-5">
            ...
            <ul className="mt-3 space-y-2.5 text-sm leading-6 text-slate-600 dark:text-slate-300">
              ...
              <li className="flex gap-2.5">
                <Centang />
                Pembahasan lengkap terbuka segera setelah Anda submit.
              </li>
            </ul>
          </Card>
        </div>
        ...
        <div className="lg:sticky lg:top-24 lg:self-start">
          <Card className="p-6">
              ...
              <>
                <div className="rounded-2xl bg-violet-50 p-5 dark:bg-violet-950/30">
                  <div className="flex items-center gap-2">
                    ...
                    <h2 className="text-base font-extrabold text-violet-900 dark:text-violet-200">
                      TACTIX Premium
                    </h2>
                  </div>

                  <p className="mt-2 text-sm leading-6 text-violet-800 dark:text-violet-300">
                    Buka seluruh Try Out 2–10 dengan satu akses Premium.
                  </p>

                  <div className="mt-4 space-y-2 text-sm text-violet-800 dark:text-violet-300">
                    <p>✓ 9 paket try out premium</p>
                    <p>✓ 990 soal latihan</p>
                    <p>✓ Simulasi CAT</p>
                    <p>✓ Pembahasan soal</p>
                    <p>✓ Riwayat dan analisis hasil</p>
                  </div>
                </div>

                <div className="mt-4 flex flex-col gap-2">
                  <ButtonLink href="/upgrade" className="w-full">
                    🔒 Beli Paket Premium
                  </ButtonLink>
            ...
            <dl className="mt-6 space-y-3 border-t border-slate-200 pt-5 text-sm dark:border-slate-800">
              <BarisInfo
                label="Jumlah soal"
                nilai={`${stat.total} soal`}
              />

              <BarisInfo
                label="Durasi"
                nilai={`${paket.durasiMenit} menit`}
              />

              <BarisInfo
                label="Skor maksimal"
                nilai={String(SKOR_MAKS_TOTAL)}
              />
                    ...
                    <div className="flex items-center justify-between gap-3">
                      <div>
                        ...
                        <p className="mt-0.5 text-xs text-slate-500 dark:text-slate-400">
                          ...
                          {r.selesaiPada
                            ? ` · ${r.selesaiPada.toLocaleDateString(
                                "id-ID",
                                {
                                  day: "numeric",
                                  month: "short",
