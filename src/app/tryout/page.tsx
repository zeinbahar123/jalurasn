...
export const metadata: Metadata = {
  title: "Try Out SKD CPNS 2026 — TACTIX",
  description:
    "Simulasi Try Out SKD CPNS 2026 TACTIX. Try Out 1 gratis, Try Out 2–10 tersedia untuk pengguna Premium. 110 soal per paket, durasi 100 menit, penilaian bergaya CAT.",
  keywords: [
  ...
  openGraph: {
    ...
    title: "Try Out SKD CPNS 2026 — TACTIX",
    description:
      "Try Out 1 gratis. Try Out 2–10 tersedia untuk pengguna Premium. Latihan SKD dengan simulasi bergaya CAT.",
    images: [...OG_GAMBAR],
  },
};
...
export default async function DaftarTryOut() {
  ...
  if (userId) {
    const [user, userAttempts] = await Promise.all([
  ...
  const totalSelesai = attempts.filter(
    (a) => a.status !== "BERLANGSUNG"
  ).length;

  const skorTertinggi = attempts.reduce(
    (m, a) => Math.max(m, a.skorTotal),
    0
  );
    ...
    <div className="mx-auto max-w-6xl px-4 py-12 sm:px-6">
      <div className="max-w-3xl">
        <Badge className="bg-indigo-500/10 text-indigo-700 ring-indigo-600/20 dark:text-indigo-300 dark:ring-indigo-400/30">
          Paket 1 Gratis · Paket 2–10 Premium · {PAKET.length * TOTAL_SOAL} soal
        </Badge>
        ...
        <p className="mt-4 text-base leading-7 text-slate-600 dark:text-slate-300">
          Uji kemampuan Anda dengan simulasi SKD bergaya CAT. Setiap paket
          berisi {TOTAL_SOAL} soal dengan durasi 100 menit dan penilaian yang
          sama berdasarkan karakteristik TWK, TIU, dan TKP.
        </p>
      </div>
        ...
        <Card className="mt-8 flex flex-col items-start gap-4 p-6 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h2 className="text-base font-bold text-slate-900 dark:text-white">
              Masuk untuk menyimpan hasil latihan
            </h2>
            ...
            <p className="mt-1 max-w-xl text-sm text-slate-600 dark:text-slate-300">
              ...
              kelemahan Anda tersimpan dan bisa dibuka dari perangkat mana pun.
            </p>
          </div>

          <div className="flex shrink-0 gap-2">
            <ButtonLink
              href="/masuk?lanjut=%2Ftryout"
              varian="sekunder"
            >
              Masuk
            </ButtonLink>

            <ButtonLink href="/daftar?lanjut=%2Ftryout">
              Daftar Gratis
            </ButtonLink>
          </div>
        </Card>
        ...
        <div className="mt-8 grid gap-4 sm:grid-cols-3">
          ...
          <Card className="p-5">
            ...
            <p className="mt-1 text-sm leading-6 text-slate-600 dark:text-slate-300">
              {isPremium
                ? "Semua paket Try Out 1–10 dapat Anda kerjakan."
                : "Paket 1 gratis. Upgrade Premium untuk membuka Paket 2–10."}
            </p>
      ...
      <div className="mt-10 space-y-4">
        {PAKET.map((p) => {
          ...
          // Paket 1 GRATIS.
          // Paket 2–10 PREMIUM.
          const paketPremium = p.nomor > 1;

          // User boleh mengerjakan apabila:
          // 1. Paket 1, atau
          // 2. User sudah Premium.
          const bolehMulai = !paketPremium || isPremium;
              ...
              <div className="flex flex-col gap-6 p-5 sm:p-6 lg:flex-row lg:items-center">
                <div className="flex-1">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {paketPremium ? (
                      <Badge className="bg-violet-500/15 text-violet-700 ring-violet-600/25 dark:text-violet-300">
                        Premium
                      </Badge>
                    ) : (
                  ...
                  <div className="mt-4 flex flex-wrap items-center gap-x-5 gap-y-2 text-xs text-slate-500 dark:text-slate-400">
                    ...
                    {sudahPernah ? (
                      <span className="font-semibold text-indigo-600 dark:text-indigo-400">
                        Skor terbaik Anda: {info!.terbaik} ·{" "}
                        {formatDurasiPanjang(info!.durasiTerbaik)}
                      </span>
                    ) : null}
                  </div>
      ...
      {!isPremium ? (
        <Card className="mt-10 overflow-hidden border-indigo-200 bg-indigo-50/50 dark:border-indigo-900 dark:bg-indigo-950/30">
          <div className="flex flex-col gap-5 p-6 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <Badge className="bg-indigo-500/10 text-indigo-700 ring-indigo-600/20 dark:text-indigo-300">
                TACTIX Premium
              </Badge>

              <h2 className="mt-3 text-xl font-extrabold text-slate-900 dark:text-white">
                Buka seluruh Try Out 2–10
              </h2>

              <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600 dark:text-slate-300">
                Satu akses Premium membuka seluruh paket Premium TACTIX,
                sehingga Anda bisa berlatih berulang kali dan mengukur
      ...
      <div className="mt-10">
        <Callout
          nada="peringatan"
          judul="Soal ini latihan, bukan bocoran"
        >
          Semua soal disusun mandiri berdasarkan kisi-kisi SKD yang terbuka
          untuk umum. Tidak ada satu pun soal resmi BKN di sini — siapa pun
          yang menjanjikan bocoran soal CPNS patut Anda curigai.
        </Callout>
      </div>
    </div>
  );
